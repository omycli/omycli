...
const archiveName = process.argv[2] ? true : false;
...
console.log('');
console.log(chalk.cyan('  准备开发打包 ...'));
console.log('');
console.log(chalk.cyan('  打包路径: ') + chalk.cyan.inverse(buildPath));
    ...
    if (archiveName) {
        ...
        if (archiveName === 'true') {
            outputPath = path.join(
                outputPath,
                path.basename(archivePath) + '.' + archiveType
            );
        } else {
        ...
        output.on('close', function() {
            console.log(
                chalk.yellow('  Archive finished, output: ') +
                    chalk.bgYellow(outputPath) +
                    ' ' +
                    chalk.bgYellow(archive.pointer() + ' bytes')
            );
            console.log('');
        });
